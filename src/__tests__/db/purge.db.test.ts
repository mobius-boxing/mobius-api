...
describeIfLocalDb("T4 purge, integrity and pre-C1 against real Postgres", () => {
  ...
  const createCompany = async (label: string): Promise<Ref> => {
    ...
    const company = track(
      await one<Ref>(`select id, uuid::text as uuid from companies where slug = ?`, [slug]),
    );
    companyIds.push(company.id);
    return company;
  };

  const createUser = async (label: string, companyId: number | null): Promise<Ref> => {
    const user = track(
      await one<Ref>(
        `insert into users (email, password, "firstName", "lastName", "companyId", role)
         values (?, 'x', 'zz', 'jest', ?, ?) returning id, uuid::text as uuid`,
        [`zz-jest-t4-${label}-${RUN}@example.test`, companyId, companyId === null ? "superAdmin" : "member"],
      ),
    );
  ...
  const insert = async (sql: string, bindings: Knex.RawBinding[]): Promise<Ref> =>
    track(await one<Ref>(`${sql} returning id, uuid::text as uuid`, bindings));
  ...
  const workflow = (companyId: number, createdBy: number | null): Promise<Ref> =>
    insert(
      `insert into nf_workflows (uuid, "companyId", name, "createdByUserId") values (?, ?, ?, ?)`,
      [randomUUID(), companyId, `zz-jest-t4-wf-${RUN}-${randomUUID()}`, createdBy],
    );
  ...
  const member = (groupId: number, userId: number): Promise<Ref> =>
    insert(`insert into countdown_group_members ("groupId", "userId") values (?, ?)`, [groupId, userId]);
  ...
  const valueOf = async (table: string, column: string, ref: Ref): Promise<unknown> =>
    (await rows<Row>(`select ?? as v from ?? where uuid = ?`, [column, table, ref.uuid]))[0]?.v;

  const exists = async (table: string, ref: Ref): Promise<boolean> =>
    (await rows(`select 1 from ?? where uuid = ?`, [table, ref.uuid])).length > 0;

  /** Superuser session with foreign-key triggers off: the only way to plant an orphan an FK would reject. */
  const withoutForeignKeys = async (sql: string, bindings: unknown[]): Promise<void> => {
  ...
  beforeAll(async () => {
    ...
    admin = new Client({
      ...
      database: c.database,
      user: adminUser,
      password: process.env.SQL_ADMIN_USER ? process.env.SQL_ADMIN_PASSWORD : c.password,
    });
    await admin.connect();
  ...
  afterAll(async () => {
    try {
      const failures = await runAllSteps([
        ...
        ["planted files", () => inMaintenance(`delete from files where uuid = any(?)`, [uuids])],
        ["planted nf_workflows", () => inMaintenance(`delete from nf_workflows where uuid = any(?)`, [uuids])],
        ["nf_workflows", () =>
          inMaintenance(`delete from nf_workflows where "companyId" = any(?)`, [[...companyIds, MISSING_COMPANY_ID]])],
        ["companies", () => inMaintenance(`delete from companies where id = any(?)`, [companyIds])],
        ["superAdmins", () => inMaintenance(`delete from users where id = any(?)`, [superAdminIds])],
        ["audit_logs", () =>
          inMaintenance(
            `delete from audit_logs where "companyId" = any(?) or "entityUuid" = any(?)`,
            [[...companyIds, MISSING_COMPANY_ID], uuids],
          )],
        ["snapshot file", async () => snapshotFile && fs.rmSync(snapshotFile, { force: true })],
      ]);
  ...
  describe("purgeUser follows each reference's rule (AC-21)", () => {
    ...
    it("refuses while countdown_documents.uploadedBy (RESTRICT) holds the user, before any write", async () => {
      ...
      await expect(purge).rejects.toThrow("countdown_documents.uploadedBy (1 row)");

      expect(await exists("users", uploaderA)).toBe(true);
      expect(await valueOf("files", "uploadedBy", f.uploaderFile)).toBe(uploaderA.id);
    ...
    it("deletes CASCADE rows, nulls SET NULL and declared nf_* values, and touches no other user's rows", async () => {
      ...
      expect(await exists("countdown_document_assignments", f.assignment)).toBe(false);
      expect(await exists("countdown_group_members", f.membership)).toBe(false);
      expect(await valueOf("files", "uploadedBy", f.fileA)).toBeNull();
      expect(await valueOf("nf_workflows", "createdByUserId", f.workflowA)).toBeNull();
    ...
    it("purges a superAdmin's references in every company", async () => {
      ...
      expect(await exists("countdown_group_members", f.superMembership)).toBe(false);
      expect(await valueOf("nf_workflows", "createdByUserId", f.superWorkflowB)).toBeNull();
  ...
  describe("db-check-integrity finds orphans in batches (AC-20)", () => {
    ...
    it("exits 0 on the clean copy", async () => {
      const check = await integrity();
      expect(check.err).toEqual([]);
      expect(check.out[0]).toMatch(/^db-check-integrity: CLEAN; \d+ cross-plane references checked$/);
      expect(check.code).toBe(0);
    });
    ...
    it("names a manifest-declared nf_* column holding a user that does not exist, looking values up with whereIn batches", async () => {
      ...
      const core = ((table: string) =>
        db("core")(table).on("query", (q: { sql: string }) => lookups.push(q.sql))) as unknown as Knex;

      const found = await findOrphans(
        db("tenant"),
        core,
        { table: "nf_workflows", column: "createdByUserId", referencedTable: "users", referencedColumn: "id" },
        1,
      );
      ...
      await inMaintenance(`delete from nf_workflows where uuid = ?`, [orphan.uuid]);
      expect((await integrity()).code).toBe(0);
    });

    it("names a foreign-key column holding a user that does not exist", async () => {
      const planted = await file(c.id, null);
      await withoutForeignKeys(`update files set "uploadedBy" = $1 where uuid = $2`, [MISSING_USER_ID, planted.uuid]);

      const check = await integrity();
      expect(check.code).toBe(1);
      expect(check.err.join("\n")).toContain("orphans: files.uploadedBy has 1 value(s) missing from users.id");

      await withoutForeignKeys(`update files set "uploadedBy" = null where uuid = $1`, [planted.uuid]);
      expect((await integrity()).code).toBe(0);
    });
  });
  ...
  describe("db-check-integrity finds company references outside companies", () => {
    it("names a tenant company column holding a company that does not exist", async () => {
      ...
      const planted = await file(c.id, null);
      await withoutForeignKeys(`update files set "companyId" = $1 where uuid = $2`, [MISSING_COMPANY_ID, planted.uuid]);
      ...
      await withoutForeignKeys(`delete from files where uuid = $1`, [planted.uuid]);
      expect((await integrity()).code).toBe(0);
    });
  });
  ...
  describe("purgeCompany through the module hooks at shared placement (AC-22)", () => {
    it("leaves 0 rows of the company in every scoped table, and a rerun is a no-op", async () => {
      ...
      const warehouse = await insert(`insert into warehouses (company_id, name) values (?, ?)`, [c.id, `zz-jest-t4-wh-${RUN}`]);
      const location = await insert(`insert into warehouse_locations (warehouse_id, "row", col) values (?, 1, 1)`, [warehouse.id]);
      ...
      for (const t of await discoverScopedTables(db("core"))) {
        const { n } = t.via === "company"
          ? await one<{ n: number }>(`select count(*)::int as n from ?? where ?? = ?`, [t.table, t.column, c.id])
          : await one<{ n: number }>(`select count(*)::int as n from ?? where ?? = ?`, [t.table, t.column, warehouse.id]);
        if (n > 0) survivors.push(`${t.table}: ${n}`);
      }
      expect(survivors).toEqual([]);
      for (const [table, ref] of [
        ["companies", c], ["users", user], ["warehouse_locations", location],
        ["countdown_document_assignments", assigned], ["nf_workflows", flow], ["files", attachment],
      ] as const) {
        expect([table, await exists(table, ref)]).toEqual([table, false]);
      }

      await expect(purgeCompany(c.id)).resolves.toEqual({ companyDeleted: false, ledgerRowsDeleted: 0 });
    });
  });

  describe("db-check-integrity --pre-c1 (AC-84)", () => {
    let a: Ref, b: Ref, memberA: Ref, otherA: Ref, memberB: Ref, superAdmin: Ref;
    ...
    beforeAll(async () => {
      ...
      const companies = await rows<{ id: number; uuid: string }>(`select id, uuid::text as uuid from companies order by id`);
      snapshotFile = path.join(os.tmpdir(), `zz-jest-t4-pre-c1-${RUN}.snapshot.json`);
      fs.writeFileSync(snapshotFile, JSON.stringify({
        snapshotId: `zz-jest-${RUN}`,
        takenAt: new Date().toISOString(),
        imageCommit: "zz-jest",
        keeperIds: companies.map((co) => co.id),
        counts: { companies, scoped: {}, nullCompany: {}, setNullToUsers: {}, s3Prefixes: {} },
        attributionTuples: [{ table: "files", rowUuid: tuple.uuid, column: "uploadedBy", userEmail: `zz-jest-gone-${RUN}@example.test` }],
      }));
    });
    ...
    it("clause 1: a company outside the keepers is red", async () => {
      ...
      await inMaintenance(`delete from audit_logs where "companyId" = ?`, [extra.id]);
    ...
    it("clause 2: a row scoped to a company outside the keepers is red", async () => {
      ...
      const check = await preC1();
      expect(check.code).toBe(1);
      expect(linesOf(check.err, "pre-c1 non-keeper rows: nf_workflows (direct) 1")).toHaveLength(1);

      await inMaintenance(`delete from nf_workflows where uuid = ?`, [stray.uuid]);
      await inMaintenance(`delete from audit_logs where "companyId" = ?`, [MISSING_COMPANY_ID]);
      expect((await preC1()).code).toBe(0);
    });

    it("clause 3: an attribution tuple pointing at another company's user is red, at a same-company user green", async () => {
      await rows(`update files set "uploadedBy" = ? where uuid = ?`, [memberB.id, tuple.uuid]);
      const red = await preC1();
      expect(red.code).toBe(1);
      expect(linesOf(red.err, `pre-c1 attribution: files.uploadedBy row ${tuple.uuid}`)).toHaveLength(1);

      await rows(`update files set "uploadedBy" = ? where uuid = ?`, [otherA.id, tuple.uuid]);
      const reattributed = await preC1();
      expect(reattributed.err).toEqual([]);
      expect(reattributed.code).toBe(0);

      await rows(`update files set "uploadedBy" = null where uuid = ?`, [tuple.uuid]);
    });

    it("clause 4: a SET NULL→users value of another company or a missing user is red; a superAdmin is not another company", async () => {
      await rows(`update files set "uploadedBy" = ? where uuid = ?`, [memberB.id, plain.uuid]);
      const foreign = await preC1();
      expect(foreign.code).toBe(1);
      expect(linesOf(foreign.err, "pre-c1 user references: files.uploadedBy has 0 row(s) pointing at a missing user and 1 at a user of another company")).toHaveLength(1);

      await rows(`update files set "uploadedBy" = ? where uuid = ?`, [superAdmin.id, plain.uuid]);
      const operatingAs = await preC1();
      expect(operatingAs.err).toEqual([]);
      expect(operatingAs.code).toBe(0);

      await withoutForeignKeys(`update files set "uploadedBy" = $1 where uuid = $2`, [MISSING_USER_ID, plain.uuid]);
      const missing = await preC1();
      expect(missing.code).toBe(1);
      expect(linesOf(missing.err, "pre-c1 user references: files.uploadedBy has 1 row(s) pointing at a missing user")).toHaveLength(1);

      await withoutForeignKeys(`update files set "uploadedBy" = null where uuid = $1`, [plain.uuid]);
      expect((await preC1()).code).toBe(0);
      void memberA;
    });
  });

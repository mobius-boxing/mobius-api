...
describeIfLocalDb("Sales order approvals against the database", () => {
  ...
  /**
   * `SalesOrderDAO` reads `db("tenant")` (T8/AC-49: unscoped throws
   * `TenantNotResolvedError` outside a request). C1's shared target is the
   * core connection itself — same pattern as purge.db.test.ts.
   */
  ...
  describe("no side effects (AC-5, R-2/R-5)", () => {
    ...
    it("keeps an existing credit-limit override when financial approval is cancelled (D-7)", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
        ...
        const after = await rawOrder(id);
        expect(after.creditLimitOverrideAt).toEqual(
          before.creditLimitOverrideAt,
        );
    ...
    it("leaves the other machine's four columns null when only one is stamped (AC-4)", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
  ...
  describe("no state guards (AC-6, R-3)", () => {
    it("approves and cancels both machines on a fulfilled AND voided order", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
        const before = await rawOrder(id);
        ...
        for (const machine of ["commercial", "financial"] as const) {
          for (const action of ["approve", "cancel"] as const) {
            const updated = await dao.setApproval(
              id,
              machine,
              action,
              "user@x",
  ...
  describe("unconditional re-stamp (AC-7, R-4)", () => {
    it("re-approves an already-approved machine and never refuses", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
  ...
  describe("event log (AC-11, AC-21)", () => {
    it("inserts exactly one row per successful action, with the acting user", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
    ...
    it("writes no event row for an order that no longer exists (AC-11, AC-12)", () =>
      runAsCoreTenant(async () => {
    ...
    it("takes its event rows with the order when it is deleted (L-006, AC-21)", () =>
      runAsCoreTenant(async () => {
        const id = await createStampedOrder();
        await dao.setApproval(id, "commercial", "approve", "aprobador@x");
        expect(await eventCount(id)).toBe(1);

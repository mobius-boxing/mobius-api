...
describe('WarehouseDAO', () => {
  ...
  beforeEach(() => {
    ...
    (mockTrx as any).fn = { now: jest.fn().mockReturnValue(new Date().toISOString()) };
    ...
    (mockKnex as any).transaction = jest.fn().mockImplementation(async (callback: any) => {
  ...
  describe('getByUuid', () => {
    it('should return warehouse by UUID', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('warehouses.uuid', testData.uuid);
      expect(result?.uuid).toBe(testData.uuid);
    });
  ...
  describe('update', () => {
    it('should update warehouse name without changing grid', async () => {
      ...
      const result = await dao.update(testData.id, { name: 'Updated Warehouse' });
    ...
    it.skip('should recreate grid locations when dimensions change', async () => {
      ...
      const result = await dao.update(testData.id, { gridRows: 3, gridCols: 3 });

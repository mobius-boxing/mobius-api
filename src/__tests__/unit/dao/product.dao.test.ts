...
describe('ProductDAO', () => {
  ...
  describe('getByUuid', () => {
    it('should return product by UUID', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('products.uuid', testData.uuid);
      expect(result?.code).toBe(testData.code);
    });
  ...
  describe('update', () => {
    it('should update product by numeric ID', async () => {
      const testData = createTestProduct();
      const updatedData = { ...testData, code: 'UPDATED_CODE', description: 'Updated description' };
    ...
    it('should update customerId when provided', async () => {
      const testData = createTestProduct();
      mockQueryBuilder.returning.mockResolvedValue([{ ...testData, customerId: 5 }]);
  ...
  describe('getAll', () => {
    ...
    it('should order by createdAt descending', async () => {
      ...
      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('products.createdAt', 'desc');
    });

...
async function testEmailService() {
  ...
  try {
    const resetToken = await tokenService.generateEmailToken(1, 'password_reset');
    console.log('   Token generated:', resetToken.token.substring(0, 20) + '...');
    ...
    console.log('   Token verified after invalidation:', verifiedAgain !== null);
  ...
  try {
    ...
    console.log('   Token generated:', verifyToken.token.substring(0, 20) + '...');
  ...
  if (!emailService.isReady()) {
    ...
    console.log('To enable actual email sending, add SENDGRID_API_KEY to your .env file.');
  } else {

...
export class ConsumableStockController extends BaseCrudController<IConsumableStock> {
  ...
  protected async buildCreateDTO(
    ...
    const inputDTO = new ConsumableStockCreateInputDTO(req.body).build();
  ...
  protected async buildUpdateDTO(
    ...
    const inputDTO = new ConsumableStockUpdateInputDTO(req.body).build();
    const validation: IInputValidator = await inputValidator(inputDTO);
    if (!validation.success) {
      req.statusCode = 400;
      next(new Error(validation.message));
  ...
  protected async beforeCreate(
    ...
    const consumableSupplyId = await this._consumableSupplyDAO.getIdByUuid(
      inputDTO.consumableSupplyUuid,
    );
    if (!consumableSupplyId) {
      res
        .status(400)
        .json({ success: false, message: "Consumable supply not found" });
      return null;
    }

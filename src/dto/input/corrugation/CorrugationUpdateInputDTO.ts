    ...
    collect((field) => {
      ...
      if (this.layers !== undefined) {
        this.layers = this.layers.map((layer, index) => ({
          // Identity reference for the diff-and-upsert path (audit P1b). This
          // map rebuilds each layer, so omitting `uuid` here would silently
          // turn every corrugation save back into a full delete-and-reinsert —
          // the exact bug P1b removed. Never written: the DAO mints the uuid of
          // any row it actually inserts, and an unknown one becomes a new row.
          uuid: field(`layers.${index}.uuid`, () =>
            optionalUuid(layer?.uuid, CORRUGATION_LABELS.layerUuid),
          ),

...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable("toolings", function (table) {
    ...
    // Foreign keys
    table.foreign("manufacturerId").references("id").inTable("manufacturers").onDelete("SET NULL");
    table.foreign("supplierId").references("id").inTable("suppliers").onDelete("SET NULL");
    table.foreign("toolingTypeId").references("id").inTable("tooling_types").onDelete("RESTRICT");
